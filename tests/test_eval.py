...
class TestSchemaValidation:
    def test_jsonl_schema_valid(self):
        ...
        for s in samples:
            assert s.task_type == "coding_agent"
...
class TestGraderIntegration:
    ...
    def test_grade_returns_metrics(self):
        mock_output = (
            "FILES: [tests/test_login.py::test_invalid_token_message]\n"
            "TESTS: [tests/test_login.py::test_invalid_token_message]"
        )
        first_graded = None
        for sample in self.samples:
            result = EvalResult(
            ...
            assert "file_selection_recall" in graded.metric_values
            assert "patch_present" in graded.metric_values
            assert "test_plan_recall" in graded.metric_values
            if first_graded is None:
                first_graded = graded

        # The mock output matches code_0001 gold, so first sample should
        # have non-zero file_selection_recall and test_plan_recall.
        assert first_graded.metric_values["file_selection_recall"] > 0
        assert first_graded.metric_values["test_plan_recall"] > 0
